 ...
 * Modified to import ALL categories and channels without any exceptions.
 */
final class Importer {
  ...
  private static function fetch(string $url): array {
    ...
    curl_setopt_array($ch, [
      ...
      CURLOPT_FOLLOWLOCATION => true, // تم تفعيل تتبع التحويلات لضمان جلب البيانات
      CURLOPT_HTTPHEADER => ['Accept: application/json'],
      CURLOPT_USERAGENT => 'OnPlusImporter/2.0'
    ]);
    ...
    if ($body === false || $code < 200 || $code >= 300) {
        throw new \RuntimeException('Import source failed: ' . $err . ' HTTP ' . $code);
    }
    
    // استخدام JSON_INVALID_UTF8_IGNORE لضمان عدم توقف السكربت بسبب أخطاء الترميز
    $json = json_decode($body, true, 512, JSON_THROW_ON_ERROR | JSON_INVALID_UTF8_IGNORE);
  ...
  private static function first(array $a, array $keys): mixed { 
      foreach ($keys as $k) { 
  ...
  private static function isUrl($v): ?string { 
      $v = self::text($v); 
      return $v && filter_var($v, FILTER_VALIDATE_URL) ? $v : null; 
  }

  // تم تعديل الدالة لعدم استثناء أي بيانات، ليتم حفظ الـ raw_data كاملاً كما هو
  ...
  public static function importCategories(PDO $pdo): array {
    $items = self::fetch(self::CATEGORIES_URL); 
    $countC = 0; 
    $countCh = 0;
    ...
    try {
      foreach ($items as $item) {
        ...
        // إذا لم يكن هناك ID، ننشئ ID تلقائي بدلاً من تخطي التصنيف
        $external = self::text($item['id'] ?? null); 
        if (!$external) {
            $external = 'cat-auto-' . substr(hash('sha256', json_encode($item)), 0, 16);
        }
        ...
        $stmt->execute([
            $external, 
            (string)self::first($item, ['name', 'title']) ?: 'غير مسمى', 
            self::isUrl(self::first($item, ['image', 'image_url', 'icon', 'logo'])), 
            self::text(self::first($item, ['description', 'desc'])), 
            (int)(self::first($item, ['order', 'position', 'sort']) ?? 0), 
            self::CATEGORIES_URL, 
            json_encode(self::sanitize($item), JSON_UNESCAPED_UNICODE)
        ]);
        ...
        if (!is_array($channels)) continue; // ننتقل للتصنيف التالي إذا لم تكن هناك قنوات (لكن التصنيف تم حفظه بنجاح)
        
        foreach ($channels as $ch) {
          if (!is_array($ch)) continue;
          
          $cid = self::text($ch['id'] ?? null); 
          if (!$cid) {
              $cid = 'cat-' . $external . '-' . substr(hash('sha256', json_encode($ch)), 0, 24);
          }
          ...
          $q->execute([
              $cid, 
              $categoryId, 
              (string)(self::first($ch, ['name', 'title']) ?? 'غير مسمى'), 
              self::isUrl(self::first($ch, ['logo', 'logo_url', 'image', 'icon'])), 
              self::streamUrl($ch), 
              self::text(self::first($ch, ['streamType', 'stream_type', 'type', 'format'])), 
              self::text(self::first($ch, ['description', 'desc'])), 
              true, 
              (int)(self::first($ch, ['order', 'position', 'sort']) ?? 0), 
              !empty(self::first($ch, ['isLive', 'is_live', 'live'])), 
              self::CATEGORIES_URL, 
              json_encode(self::sanitize($ch), JSON_UNESCAPED_UNICODE)
          ]);
          $countCh++;
        }
      }
  ...
  public static function importMatches(PDO $pdo): array {
    $items = self::fetch(self::MATCHES_URL); 
    $count = 0;
    ...
    try {
      foreach ($items as $m) {
        ...
        $id = self::text($m['id'] ?? null); 
        if (!$id) {
            $id = 'match-' . substr(hash('sha256', json_encode($m)), 0, 24);
        }
        
        $date = (string)(self::first($m, ['date', 'match_date']) ?? ''); 
        $time = (string)(self::first($m, ['time', 'match_time']) ?? '');
        $dt = null; 
        
        try { 
            if (!empty(trim($date . ' ' . $time))) {
                $dt = (new \DateTime(trim($date . ' ' . $time), new \DateTimeZone('UTC')))->format('c'); 
            }
        } catch (\Throwable $e) {}
        ...
        $q->execute([
            $id, 
            (string)(self::first($m, ['homeTeam', 'home_team']) ?? 'غير مسمى'), 
            (string)(self::first($m, ['awayTeam', 'away_team']) ?? 'غير مسمى'), 
            self::isUrl(self::first($m, ['homeLogo', 'home_logo'])), 
            self::isUrl(self::first($m, ['awayLogo', 'away_logo'])), 
            self::text(self::first($m, ['league', 'competition'])), 
            self::isUrl(self::first($m, ['leagueLogo', 'league_logo'])), 
            $dt, 
            $status, 
            self::first($m, ['homeScore', 'score_home']), 
            self::first($m, ['awayScore', 'score_away']), 
            self::streamUrl($m), 
            self::text(self::first($m, ['streamType', 'stream_type', 'type'])), 
            self::MATCHES_URL, 
